...
app.post('/webhook/github', verifyGitHubSignature, async (req, res) => {
  ...
  try {
    ...
    const channelPrefix = repoName.toLowerCase();
...
async function handlePushEvent(guild, channelPrefix, payload) {
  const channelName = `${channelPrefix}-commits`;
  ...
  console.log(`Posted ${commits.length} commits to ${channelName}`);
}

async function handleReleaseEvent(guild, channelPrefix, payload) {
  if (payload.action !== 'published') return;

  const channelName = `${channelPrefix}-releases`;
  const channel = guild.channels.cache.find(c => c.name === channelName);
  ...
  await channel.send({ embeds: [embed] });
  console.log(`Posted release ${release.tag_name} to ${channelName}`);
}
